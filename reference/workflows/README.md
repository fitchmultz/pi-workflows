# Source Harness Development Workflows

[![Source Harness](https://img.shields.io/badge/Model%20Code-Plugin-purple)](https://model.ai/code)
[![GitHub Stars](https://img.shields.io/github/stars/shinpr/workflows?style=social)](https://github.com/shinpr/workflows)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](https://github.com/shinpr/workflows/pulls)

Source Harness can explore a codebase deeply. On non-trivial work, the harder problem is convergence. While designing an account-recovery flow, Model may find a real inconsistency in token handling and spend most of the design on it, leaving the requested recovery behavior vague.

workflows keeps that work tied to an agreed result. It records the outcome and its exclusions, then creates only the design and verification artifacts the change needs. A separate review checks the finished implementation against that outcome, while Model still decides how to get there from repository evidence.

Use Source Harness directly when the outcome and safe implementation boundary are already clear. Use these workflows when a change needs scope agreement, durable design decisions, a reliable handoff between contexts, or independent verification.
...
Specialist agents contribute repository evidence and draft artifacts. The main session still owns the product scope: it applies findings needed to protect an approved requirement, contract, or observable behavior, and can decline findings that only add work. Product changes and major design changes return to the user. Reversible implementation choices remain with Model.

Because the process is packaged as a Source Harness plugin, a team can apply the same boundaries across repositories without prescribing Model's steps.

---

## Quick Start

Requires a Source Harness release with plugin marketplace support.

### Choose a path

| What do you need? | Start with | Plugin |
|---|---|---|
| Deliver a backend, API, CLI, or general change end to end | `/recipe-implement` | `workflows` |
| Design a backend or general change before implementation | `/recipe-design` | `workflows` |
| Design and build a React / TypeScript frontend | `/recipe-front-design` → `/recipe-front-plan` → `/recipe-front-build` | `workflows-frontend` |
| Deliver a backend and React frontend change together | `/recipe-fullstack-implement` | `workflows-fullstack` |
| Review an implementation against its design | `/recipe-review` or `/recipe-front-review` | `workflows` or `workflows-frontend` |
| Investigate a problem before choosing a fix | `/recipe-diagnose` | Any workflow plugin |
| Document an existing system from its code | `/recipe-reverse-engineer` | `workflows` or `workflows-fullstack` |
| A throwaway experiment or prototype | Use Source Harness directly | None |

### Common setup

```bash
# 1. Start Source Harness
model

# 2. Add the marketplace
/plugin marketplace add shinpr/workflows
```
...
```bash
# Backend or general
/plugin install workflows@workflows
/recipe-implement "Add rate limiting to the public API"

# Frontend
/plugin install workflows-frontend@workflows
/recipe-front-design "Add account recovery screens"

# Full-stack
/plugin install workflows-fullstack@workflows
/recipe-fullstack-implement "Add user authentication with JWT + login form"
```

Install only one workflow plugin. `workflows-fullstack` already contains the backend and frontend workflows. If you previously used full-stack recipes from `workflows`, migrate to `workflows-fullstack`.
...
Source Harness supports project-scoped marketplaces and plugins. Commit the resulting `.model/settings.json` so contributors are prompted to use the same workflow plugin.

```bash
model plugin marketplace add shinpr/workflows --scope project
model plugin install workflows-fullstack@workflows --scope project
```

Replace `workflows-fullstack` with the plugin that matches the repository. See the [Source Harness plugin documentation](https://code.model.com/docs/en/discover-plugins#configure-team-marketplaces) for project and managed installation options.
...
<details>
<summary>Use the guidance without the workflow (skills-lite)</summary>

If you already have orchestration through custom prompts or CI and want only the best-practice guides, use `skills-lite`. If you want Model to plan, execute, and verify a change end to end, install one of the workflow plugins instead.
...
> **Do not install `skills-lite` alongside a workflow plugin.** They share the same skills, and duplicate descriptions can cause Source Harness to ignore skills after reaching its context limit.

```bash
/plugin install skills-lite@workflows
```

To switch between plugin types:

```bash
# skills-lite -> workflows
/plugin uninstall skills-lite@workflows
/plugin install workflows@workflows

# workflows -> skills-lite
/plugin uninstall workflows@workflows
/plugin install skills-lite@workflows
```
...
- [model-code-discover](https://github.com/shinpr/model-code-discover): turns feature ideas into evidence-backed PRDs.
- [metronome](https://github.com/shinpr/metronome): detects shortcut-taking behavior and asks Model to follow the defined procedure.
...
```bash
/plugin install discover@workflows
/plugin install metronome@workflows
/plugin install linear-prism@workflows
/plugin install pr-review@workflows
```
