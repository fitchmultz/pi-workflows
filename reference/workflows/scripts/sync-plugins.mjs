...
const MARKETPLACE_PATH = join(ROOT, '.plugin', 'marketplace.json')
...
async function generatePlugin(entry, baseDir) {
  ...
  await mkdir(join(targetDir, '.plugin'), { recursive: true })
  await writeFile(
    join(targetDir, '.plugin', 'plugin.json'),
...
async function applyNamespaceRewrites(entry, targetDir) {
  ...
  // (e.g. rewriting "workflows:" before "workflows-frontend:" would corrupt the latter).
