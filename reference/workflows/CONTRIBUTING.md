...
Thank you for your interest in contributing to Source Harness Workflows.
...
1. **Be a working Source Harness plugin** — Installable via `/plugin install`
2. **Have a public repository** with a README explaining what it does and why
3. **Be actively maintained** — Recent commits or responsive issue activity, compatible with current Source Harness
...
2. Add your plugin to `.plugin/marketplace.json`:
   ```json
   {
...
4. [Open a Pull Request using the external plugin template](https://github.com/shinpr/workflows/compare/main...main?template=external-plugin.md) and fill in the required information
...
The bundled plugins (`workflows`, `workflows-frontend`, `skills-lite`) live in subdirectories at the repo root, but those subdirectories are **generated**. The canonical sources are the top-level `agents/` and `skills/` directories — edit only there. The per-plugin curation lives in `.plugin/marketplace.json` (`agents` / `skills` arrays per entry).
...
Requires Node.js >= 22 and `pnpm`. The pre-commit hook runs `pnpm sync` to regenerate the subdirectories and then `model plugin validate` on each, so committing changes to `agents/`, `skills/`, or `marketplace.json` will keep everything consistent automatically. Run `pnpm sync` manually if you need to inspect the result without committing.
