...
| Neither exists + Design Doc exists | No plan, no Consumed Task Set, but `docs/design/*.md` exists | Invoke work-planner to create a work plan, then run document-reviewer (`workflows-frontend:document-reviewer`, doc_type: WorkPlan). Run Review Resolution through its correction re-review, escalation, and convergence transitions, using work-planner for rerouted corrections; then present the resolved plan for batch approval before task materialization |
...
- `subagent_type`: "workflows-frontend:task-decomposer"
...
1. **EXECUTE**: invoke Agent tool (subagent_type: "workflows-frontend:task-executor-frontend") → Record the current HEAD as `diffBase`, pass `task_file: [path]`, and receive the structured response
...
- code-verifier (subagent_type: "workflows-frontend:code-verifier") → verify the completed implementation against the resolved `doc_type` and `document_path`
- security-reviewer (subagent_type: "workflows-frontend:security-reviewer") → review the completed implementation against the same typed `governingDocuments`
