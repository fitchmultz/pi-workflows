...
| Neither exists + Design Doc exists | No plan, no Consumed Task Set, but `docs/design/*.md` exists | Invoke work-planner to create a work plan, then run document-reviewer (`workflows:document-reviewer`, doc_type: WorkPlan). Run Review Resolution through its correction re-review, escalation, and convergence transitions, using work-planner for rerouted corrections; then present the resolved plan for batch approval before task materialization |
...
- `subagent_type`: "workflows:task-decomposer"
...
|-----------------|----------|---------------|
| `*-backend-task-*` | workflows:task-executor | workflows:quality-fixer |
| `*-frontend-task-*` | workflows-frontend:task-executor-frontend | workflows-frontend:quality-fixer-frontend |
...
- code-verifier (subagent_type: "workflows:code-verifier") → verify the completed implementation against each resolved `doc_type` and single `document_path`
- security-reviewer (subagent_type: "workflows:security-reviewer") → review the completed implementation against the typed `governingDocuments` list
