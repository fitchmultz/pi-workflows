...
1. **EXECUTE**: invoke Agent tool (subagent_type: "workflows:task-executor") → Record the current HEAD as `diffBase`, pass `task_file: [path]`, and receive the structured response
...
- code-verifier (subagent_type: "workflows:code-verifier") → verify the completed implementation against the resolved `doc_type` and `document_path`
- security-reviewer (subagent_type: "workflows:security-reviewer") → review the completed implementation against the same typed `governingDocuments`
