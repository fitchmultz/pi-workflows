...
|-------|--------------------|
| Number and kind of targets to change | grep/find over the affected files |
...
| Whether verification support already exists | find for the area's existing test harness |
| What remains unknown | agent_browser_web_search for a dependency's current capability; anything the above cannot resolve |
