...
Execute the external-resource-context skill (/skill:external-resource-context) before running the external resource hearing in monorepo-flow.md.
...
- Pass all Design Docs to work-planner (agent: "work-planner") with vertical slicing instruction
- Pass the Work Plan to document-reviewer (`doc_type: WorkPlan`) and request batch approval only after the review passes

### 4. Register All Flow Steps Using todo_list (MANDATORY)

After scale determination, use todo_list to register each design/planning step and the implementation, verification, cleanup, and report phases. Complete registration before invoking subagents; mark and advance the active phase with todo_list.

## After requirement-analyzer [Stop]

Execute the requirement-convergence skill (/skill:requirement-convergence) before running the hearing protocol.
...
**Subagent routing by task filename** (see monorepo-flow.md reference):
```
...
**Rules**:
1. Execute ONE task completely before starting next (each task goes through the full 4-step cycle via subagent tool, using the correct executor per filename pattern)
...
- code-verifier (agent: "code-verifier") → verify the completed implementation against each resolved `doc_type` and single `document_path`
- security-reviewer (agent: "security-reviewer") → review the completed implementation against the typed `governingDocuments` list
...
After acceptance-test-generator execution, when invoking work-planner (agent: "work-planner"), communicate:
