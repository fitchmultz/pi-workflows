...
**First Action**: Register Steps 1-10 using todo_list before any execution.
...
Invoke security-reviewer using subagent tool:
...
This review command authorizes analysis; use ask_question to obtain separate implementation authority. The batch option is **"approve all proposed `apply` routes"** and its scope consists exclusively of those routes. Collect an explicit decision for each `user_decision_required` item. When the approved change set is empty, proceed directly to Step 10.
...
1. Invoke technical-designer-frontend in update mode using subagent tool:
   - `agent`: "technical-designer-frontend"
...
Invoke task-executor-frontend using subagent tool:
...
Invoke security-reviewer using subagent tool (only if security fixes were applied):
- `agent`: "security-reviewer"
