...
2. **Run in the parent session** (multi-step loops and user dialogs): external-resource hearing via ask_question, write-set inspection, scale judgment, adjustment-context approval, adjustment edits, verification against the design source, iteration until acceptance.
3. **Stop at every `[Stop: ...]` marker** before proceeding.

## Initial Mandatory Tasks

**Task Registration**: Before Step 1, register the recipe's execution flow using todo_list so progress is trackable. Register Steps 1-7 below as individual tasks plus a final task "Verify completion against Completion Criteria". Update status using todo_list as each step starts and completes.
...
Execute the external-resource-context skill (/skill:external-resource-context) before running the hearing protocol.
...
Execute the documentation-criteria skill (/skill:documentation-criteria).
...
Execute the frontend-ai-guide skill (/skill:frontend-ai-guide) before planning or applying adjustment edits.
Execute the typescript-rules skill (/skill:typescript-rules) before planning or applying adjustment edits.
Execute the implementation-approach skill (/skill:implementation-approach) before planning or applying adjustment edits.
Execute the test-implement skill (/skill:test-implement) before adding or changing tests.
...
2. **Apply the edit** using Edit / Write / edit on the affected files.
3. **Verify against external sources** using whichever access method `docs/project-context/external-resources.md` declares for each axis:
   - Design origin: compare current rendering against the design source via the declared access method (e.g., design-tool MCP, agent_browser from a public URL, file read from a specification path)
...
- Invoke **quality-fixer-frontend** using subagent tool
