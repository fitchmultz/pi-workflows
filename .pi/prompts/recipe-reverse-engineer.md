...
**Task Registration**: Register phases first using todo_list, then steps within each phase as you enter it. Update status using todo_list.
...
**subagent tool invocation (per Design Doc)**:
```
agent: code-verifier
...
**subagent tool invocation (per Design Doc)**:
```
agent: technical-designer (or technical-designer-frontend for frontend Design Docs)
