...
Execute the subagents-orchestration-guide skill (/skill:subagents-orchestration-guide) before invoking agents or resolving findings.
...
Invoke `ui-analyzer` with exactly one governing source:
...
Execute the requirement-convergence skill (/skill:requirement-convergence). Build and judge the convergence record from the governing requirement source, repository analysis, and applicable UI analysis.
...
Invoke `ui-spec-designer` with exact inputs:
...
Invoke `document-reviewer` with exact inputs: `doc_type: UISpec` and `target` as the UI Spec path returned by ui-spec-designer, unchanged. `approved` presents the UI Spec with `issues: []`; `needs_revision` applies Review Resolution and re-reviews after correction; `rejected` resolves the governing-source conflict before another review. `[Stop: UI Spec approval]`.
...
2. Collect all returned paths and invoke `document-reviewer` once with exact inputs: `doc_type: ADRBatch`, `targets: [all paths]`, and `confirmed_requirement_context`. The reviewer follows the approved UI Spec cited by the ADRs when it can change the decision review.
...
Invoke `technical-designer-frontend` with exactly:
...
Invoke `design-sync` with `source_design` as the returned Design Doc path unchanged, apply Review Resolution to actionable conflicts, and report `SKIPPED` distinctly when only one Design Doc exists.
