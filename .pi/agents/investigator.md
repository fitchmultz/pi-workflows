...
tools: read, grep, find, ls, bash, agent_browser_web_search, todo_list
...
| Design Doc/ADR | find for `docs/design/*` and `docs/adr/*` matching the feature area. Read if found |
| External (agent_browser_web_search) | Search official documentation for the primary technology involved. Search for error messages if present |
