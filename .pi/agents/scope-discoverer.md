...
tools: read, grep, find, ls, bash, todo_list
...
4.5. **Unit Inventory Enumeration**
   For each discovered unit, enumerate its internal details using grep/find:
   ...
   - **Test files**: find for test files (common conventions: `*test*`, `*spec*`, `*Test*`) matching the unit's source area. Record: file path, exists=true
...
- [ ] Enumerated unit inventory (routes, test files, public exports) for each unit using grep/find
