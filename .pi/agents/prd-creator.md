...
skills: documentation-criteria, llm-friendly-context, requirement-convergence
...
6. **Verify decision-changing external value evidence**: Use agent_browser_web_search when current market, competitor, regulatory, or compatibility facts can change business value or MVP scope and the user, accepted contracts, usage evidence, and existing PRDs do not already determine that decision. Record the authoritative source and the decision it affects
...
- find for test files matching the feature area (common conventions: `*test*`, `*spec*`, `*Test*`)
- For each test file found: Read it and record test case names and what behavior they verify
- For handlers/services with no test files found via find: record as "no tests found"
